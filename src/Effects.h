...
public:
   ...
   */
  currentDisplay *getCurrentBuffer()
  {
    return &currentBuffer;
  }
  void changeTetrisText()
  {

    uint8_t firstony = 0x00;
    for (int x = 0; x < 63; x++)
    {
      firstony = 0x00;
      for (int y = 0; y < 63; y++)
      {
        if (currentBuffer.isPixel(x, y))
        {
          ...
          {
            firstony = 0x01;
            display.drawPixelRGB888(x, y, 0xff, 0xff, 0xff); // white
          }
          else if (!currentBuffer.isPixel(x - 1, y)) // edge of number
          {
            display.drawPixelRGB888(x, y, 0xff, 0xff, 0xff); // white
          }
          else if (!currentBuffer.isPixel(x + 1, y)) // other edge of number
          {
            display.drawPixelRGB888(x, y, 0xff, 0xff, 0xff); // white
          }
          else if (!currentBuffer.isPixel(x, y - 1) || (!currentBuffer.isPixel(x, y + 1)))
          {
            display.drawPixelRGB888(x, y, 0xff, 0xff, 0xff); // white
          }
          else if (!currentBuffer.isPixel(x - 1, y + 1) || !currentBuffer.isPixel(x - 1, y - 1) || !currentBuffer.isPixel(x + 1, y - 1) || !currentBuffer.isPixel(x + 1, y + 1))
          {
            display.drawPixelRGB888(x, y, 0xff, 0xff, 0xff); // white
          }
          else // middle of
          {
            display.drawPixelRGB888(x, y, 0x00, 0x00, 0x00); // black
          }
        }
      }
    }
  }
  // write one pixel with the specified color from the current palette to coordinates NOT NOW
  ...
  */
                  ...
                  uint16_t color)
  {
    ...
    savePixel(x0, y0 + r, color);
    savePixel(x0, y0 - r, color);
    savePixel(x0 + r, y0, color);
    savePixel(x0 - r, y0, color);
